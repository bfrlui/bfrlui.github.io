...
class RacingRAGEngine:
    ...
    def _initialize_knowledge_base(self):
        """
        核心 RAG 機制：加載 7 大原始數據集
        並在內存中建立以 [場次][馬號] 為 Key 的高效語義網 (Entity Linking)
        """
        print("🗄️ 正在將本地賽馬數據集載入 RAG 內存索引...")
        ...
        exceptional = self._safe_load_json("hkjc_exceptional_ultimate_20260704.json")
        trackwork = self._safe_load_json("hkjc_trackwork_perfect.json")
        vet_features = self._safe_load_json("hkjc_vet_features_v2_20260704.json")
        incidents = self._safe_load_json("hkjc_incidents_perfect.json")
        formline = self._safe_load_json("hkjc_formline_ultimate_20260704.json")
        ...
        for r_raw_key, horses_list in starters.items():
            ...
            # 🔥【效能與邏輯優化】將場次模糊匹配移至外層（每場僅執行一次），修正原本跨表關聯失效的問題
            matched_list = None
            for k in draws.keys():
                ...
                if matched_list and isinstance(matched_list, list):
                    for d_info in matched_list:
                        if str(d_info.get("檔位")).strip() == target_draw_str:
                            # 🔥【核心修正】採用鏈式 .get() 查找，完美包含「%」符號以及可能殘留的空格
                            # 這能保證不管是新清洗版還是舊版，均能完美抓取數據，告別「未知」
        ...
        print(f"🎉 成功！RAG 引擎已完成對齊，共裝配 {len(self.kb)} 個場次的語義知識庫。")
...
if __name__ == "__main__":
    ...
    # rag.invoke_deepseek_r1(race_no=1, model_name="horse-quant")