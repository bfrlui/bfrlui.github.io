...
if __name__ == "__main__":
    # 從命令列讀取 race_date、racecourse 與 total_races
    # 用法: python hkjc_vet_records.py 2026/07/08 HV [total_races]
    if len(sys.argv) < 3:
        print("❌ 請提供 race_date 與 racecourse")
        print("用法: python hkjc_vet_records.py <race_date YYYY/MM/DD> <racecourse 例如 ST/HV> [total_races]")
        sys.exit(1)
    ...
    if len(sys.argv) >= 4:
        try:
            ...
            print(f"⚠️  場次數 '{sys.argv[3]}' 無效，使用預設值 {TOTAL_RACES}")

    run_veterinary_records_v2(RACE_DATE, racecourse=RACECOURSE, total_races=TOTAL_RACES)