...
class RacingRAGEngine:
    ...
    def _clean_race_info_text(self, text):
        """移除賽事標頭中不需要的欄位：時間、分段時間。"""
        import re
        # 移除「時間 :」及其後的括號分段時間，例如 (27.93) (52.36) (1:16.24) (1:40.05)
        text = re.sub(r'時間\s*:\s*\([^)]*\)(?:\s*\([^)]*\))*', '', text)
        # 移除「分段時間 :」及其後的數值，直到「全方位賽事重溫」或字串結尾
        text = re.sub(r'分段時間\s*:\s*[\d.\s]+?(?=全方位賽事重溫|$)', '', text)
        # 移除影片/分析連結文字
        text = text.replace('全方位賽事重溫', '').replace('餘勢分析', '')
        # 清理多餘空白
        text = ' '.join(text.split())
        return text

    def _extract_race_info_from_html(self, html):
        """從 racecard/localresults 的 HTML 中提取賽事資料標頭。"""
        from bs4 import BeautifulSoup

        soup = BeautifulSoup(html, 'html.parser')

        # 優先抓取 race_tab 區塊，這是最穩定的賽事標頭來源
        race_tab = soup.select_one('div.race_tab')
        if race_tab:
            text = ' '.join(race_tab.get_text(' ', strip=True).split())
            if text and any(keyword in text for keyword in ['第', '場', '賽道', '獎金', '場地狀況', '分段時間', '時間 :']):
                return self._clean_race_info_text(text)

        # 次選擇：抓取包含賽事資訊的表格/段落
        for selector in ['div.f_fs13', 'span.f_fs13', 'p.f_fs13']:
            node = soup.select_one(selector)
            if node:
                text = ' '.join(node.get_text(' ', strip=True).split())
                if text and any(keyword in text for keyword in ['第', '場', '賽道', '獎金', '場地狀況', '分段時間', '時間 :', '賽事日期']):
                    return self._clean_race_info_text(text)
    ...
    def _fetch_race_info(self, race_no):
        ...
        若主頁 racecard 無法提供資料，則回退至 localresults 頁面。
        """
        import requests
        ...
        for url, page_name in urls:
            try:
                ...
                race_info = self._extract_race_info_from_html(res.text)
                if race_info:
                    if page_name == "localresults":
                        print(f"✅ 第 {race_no} 場賽事資料標頭已由 {page_name} 備用頁面取得。")
                    return race_info
            except Exception as e:
                print(f"⚠️ 抓取第 {race_no} 場賽事資料標頭失敗 ({page_name}): {e}")
    ...
    def _initialize_knowledge_base(self):
        ...
        for r_raw_key, horses_list in starters.items():
            ...
            # 🌟 若指定了單一場次，則只處理該場，避免對所有場次重複抓取賽事標頭
            if self.target_race is not None and race_no != self.target_race:
                continue
            ...
            for horse in horses_list:
                ...
                if career_h:
                    semantic_profile += (
                        ...
                        f"              ℹ️ {career_h.get('基本信息', '無數據')}\n"
                        f"              💪🏻 {career_h.get('實力還原指標', '無數據')}\n"
                        f"              ⚖️ {career_h.get('臨場體重特徵', '無數據')}\n"
                        f"              🏇 {career_h.get('季內走勢', '無數據')}\n"
                    )
                    # 🌟 收集前置型馬匹（放頭馬）供場級步速總結
...
if __name__ == "__main__":
    ...
    #    用法: python rag_engine_8coll.py <場次編號> <馬場代碼> [賽事日期]
    #    例:   python rag_engine_8coll.py 5 ST 2026/07/12
    target_race = 1           # 設定預設場次編號（只生成該場報告）
    ...
    if len(sys.argv) > 1:
        try:
            ...
            print(f"📥 成功讀取命令列參數：即將生成第 【{target_race}】 場賽事報告...")
        except ValueError:
            print(f"❌ 錯誤：您輸入的 '{sys.argv[1]}' 不是有效的數字！將自動切換回預設第 {target_race} 場。")
    else:
        print(f"💡 提示：您未在命令列輸入場次編號，系統自動使用預設第 【{target_race}】 場。")
    ...
    print("👉 使用提示：'python rag_engine_8coll.py 5 ST 2026/07/12' 可指定場次/馬場/日期。")

    print("="*60)

    # 2. 初始化 RAG 引擎並生成指定場次的報告
    #    （引擎依 target_race 對齊第 8 數據集檔名，確保生涯底蘊正確對齊）
    for race_no in [target_race]:
