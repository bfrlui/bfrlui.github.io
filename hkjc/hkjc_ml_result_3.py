...
# ---------------------------------------------------------
# 4. 自動繪製特徵重要性圖表 (Bar Chart)
# ---------------------------------------------------------
def plot_feature_importance(model, feature_cols, top_n=15, save_path='feature_importance.png'):
    # 提取 Gain (增益) 特徵重要性
    ...
    plt.xlabel('Importance Gain (資訊增益貢獻度)', fontsize=12)
    plt.ylabel('Features (特徵名稱)', fontsize=12)
    plt.tight_layout()
    
    # 儲存圖表檔案
    plt.savefig(save_path, dpi=300)
    print(f"\n★ 特徵重要性長條圖已成功繪製並儲存至: {save_path}")
    plt.show()
...
if __name__ == '__main__':
    ...
    # 自動繪製並儲存前 15 大特徵重要性圖表
    plot_feature_importance(final_model, feature_cols, top_n=15)